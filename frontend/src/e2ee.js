...
// Store private key as exported b64 bytes (avoids Safari IndexedDB + CryptoKey bug)
export async function initE2EE(userId, apiFetch) {
  ...
  if (!privateB64 || !publicB64) {
    // Generate extractable keypair so we can export both keys as bytes
    ...
    // Re-upload if server lost the public key
    try {
...
export async function decryptMessage(msg, myUserId, privateKey) {
  ...
  try {
    ...
    return { ...msg, content: new TextDecoder().decode(plain) };
  } catch (err) {
